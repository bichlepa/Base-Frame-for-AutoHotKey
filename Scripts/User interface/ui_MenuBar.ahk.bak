...
MenuChangeLanguage:
WhichLanguage:=A_ThisMenuItem
MsgBox, 52, % lang("Attention"), % lang("When you change the language, Base Frame needs to be restarted. All settings will be lost")
ifmsgbox,Yes
{
	for tempkey, tempval in allLangs
	{
		if (WhichLanguage = %tempval%enlangname) 
		{
			IniWrite,% tempval,settings.ini,common,UILanguage ;Save the languate ID to the settings.ini
			break
		}
	}
	Reload
